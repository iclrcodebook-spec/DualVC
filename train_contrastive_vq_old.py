...
def get_args():
    ...
    # Dataset
    parser.add_argument("--dataset", type=str, default="cora", help="Dataset (cora, citeseer, pubmed, a-computer, a-photo, ogbn-arxiv)")
    ...
    parser.add_argument("--batch_size", type=int, default=256, help="Node batch size for mini-batch training on large graphs")
...
def train_contrastive_epoch(model, dataloader_or_graph_tuple, optimizer, contrastive_criterion, device, lambda_commit_loss):
    ...
    if is_dataloader:
        for input_nodes, output_nodes, blocks in dataloader_or_graph_tuple:
            ...
            projected1_full, projected2_full, commit_loss1, commit_loss2 = model(blocks[0], batch_feats) 
...
def run(args, pre_loaded_data=None):
    set_seed(args.seed)
    ...
    if pre_loaded_data is not None:
        ...
        if args.dataset in ['cora', 'citeseer', 'pubmed','a-computer','a-photo']:
    ...
    if is_large_graph and args.batch_size > 0:
        ...
        train_dataloader_or_graph_tuple = dgl.dataloading.DataLoader(
            g, train_nid, sampler, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=0
        )
...
def main():
    ...
    for i in range(args.num_exp):
        ...
        try:
            ...
            all_experiment_mean_accuracies.append(np.nan)
